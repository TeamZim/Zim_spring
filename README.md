...
* 여행(Trip) 관리: 여행 생성, 조회, 수정, 삭제, 테마 지정, 대표 이미지 관리
* 일기(Diary) 관리: 여행별/사용자별 일기 작성, 사진(정면/후면), 오디오, 감정, 날씨, 위치 등 기록
* 방문 국가 관리: 국가별 방문/감정 기록, 국가 검색
* 감정 및 날씨 태깅: 감정 색상, 날씨 정보와 함께 기록
* 파일 업로드: 이미지·오디오 파일을 AWS S3에 업로드 및 다운로드
* 사용자 인증: 카카오 로그인 연동
* 마이페이지/통계: 방문 국가, 작성 일기, 통계 등 사용자별 정보 제공
* 기록 기반 회고: 타임라인, 대표 이미지 등 다양한 방식의 여행 회고 지원
